...
public class LoanRESTRepositoryImpl implements LoanRESTRepository {
	...
	private static final String EMI_CALCULATOR_URL = "http://localhost:8080/LoanManagementSystem/emiCalculator";
	...
	public void sendApprovalRequest(final User user, final Loan loan) {
		...
		String requestURL = "http://localhost:8080/LoanManagementSystem/loandisb";
		try {
	...
	private String generateRequestURL(final Loan loan) {
		StringBuilder urlBuilder = new StringBuilder(EMI_CALCULATOR_URL);
		return urlBuilder.append("?").append("appliedLoanAmount=" + loan.getAppliedLoanAmount()).append("&")
				.append("loanDuration=" + loan.getTenure()).append("&")
				.append("rateOfInterest=" + loan.getMonthlyInterest()).toString();
	}
